...
function toURLParams(state: FilterState): URLSearchParams {
  ...
  if (state.isToon !== undefined) params.set('isToon', String(state.isToon))
  if (state.isSpirit !== undefined) params.set('isSpirit', String(state.isSpirit))
  if (state.isUnion !== undefined) params.set('isUnion', String(state.isUnion))
  if (state.isGemini !== undefined) params.set('isGemini', String(state.isGemini))
...
export function useFilterSync() {
  ...
  useEffect(() => {
    ...
    const isToon = searchParams.get('isToon')
    const isSpirit = searchParams.get('isSpirit')
    const isUnion = searchParams.get('isUnion')
    const isGemini = searchParams.get('isGemini')
    ...
    if (isToon !== null) store.setIsToon(isToon === 'true' ? true : undefined)
    if (isSpirit !== null) store.setIsSpirit(isSpirit === 'true' ? true : undefined)
    if (isUnion !== null) store.setIsUnion(isUnion === 'true' ? true : undefined)
    if (isGemini !== null) store.setIsGemini(isGemini === 'true' ? true : undefined)
